...
$(i, j)$ 위치의 N개의 학습 데이터셋에 대한 feature embedding $X_{i j}=\lbracex_{i j}^{k}, k \in \llbracket 1, N \rrbracket\rbrace$ 이 가우시안 분포  $\mathcal{N}\left(\mu_{i j}, \mathbf{\Sigma}_{i j}\right)$ 로 부터 생성되었다고 가정한다. ($\mu_{i j}$, $\Sigma_{i j}$ 은 각각 sample mean, sample covariance)
