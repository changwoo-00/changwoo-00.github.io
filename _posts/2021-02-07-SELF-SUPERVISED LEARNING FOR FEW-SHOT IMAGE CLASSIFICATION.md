...
\begin{equation}
	\mathcal{L}\_{amdim}(f\_{g}(x\_{a}), f\_{5}(x\_{b})) = -\log \frac{\exp \lbrace\phi(f\_{g}(x\_{a}), f\_{5}(x\_{b})))\rbrace}{\sum_{\widetilde{x\_{b}} \in \mathcal{N}\_{x} \cup x\_{b}} \exp \lbrace\phi(f\_{g}(x\_{a}), f\_{5}(\widetilde{x\_{b}}))\rbrace}
\end{equation}
