...
Defect에 대한 정보를 사전에 알 수 없는 Unsupervised learning을 가정했을 때 합리적으로 생각할 수 있는 방법은 학습시 정상 이미지 전체에 집중하도록 하는 것이다.
...
detection의 경우 input $x\_{test}$ 와 resconstruction $\hat{x}\_{test}$사이의 nomalized pixel-wise difference를 anomalous score $s\_a$ 로 정하고 threshold 0.5를 기준으로 anomaly를 판정하였다. 

localization의 경우 Grad-CAM 방식을 사용하여 $z$로 부터 attention map $A\_{test}$ 을 구하고 $(\mathbf{1} - A\_{test})$를 anomalous attention map으로 사용하였다. threshold는 0.5로 설정하였다.
...
추가적으로 소수의 label 데이터가 존재하는 상황에서의 weakly supervised $\text{CAVGA}$를 제안하였다.

Fig. 2. (b)에서 $\text{CAVGA}\_w$ 의 형태를 확인 할 수 있다. 

latent variable $z$를 1차원으로 펼친 후 fully connected layer로 정상/비정상의 binary-class classifier($C$)와 binary cross entropy loss $L\_{bce}$ 를 추가하였다.
...
LAG, MVTAD dataset 에 대한 anomaly detection 결과를 각각 Table 5, Table 6에서 확인 할 수 있다.
...
Table 8에서 latent variable $z$, loss $L\_{ae}$, $L\_{cga}$에 대한 ablation test 결과를 볼 수 있다. Method 이름 뒤에 *가 붙은 경우는 1-d latent variable을 사용했음을 의미한다.
